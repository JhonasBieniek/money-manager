...
describeWithDb("patrimony analytics integration", () => {
  ...
  it("GET /v1/patrimony/benchmarks casa snapshots com benchmark_rates por ano-mês e reflete o crescimento real do patrimônio", async () => {
    ...
    expect(res.body.series[0].ipcaAccumulatedPct).not.toBeNull();
    expect(res.body.series[0].cdiAccumulatedPct).not.toBeNull();

    expect(res.body.series[1].referenceMonth).toBe(currentMonth);
    expect(res.body.series[1].patrimonyIndexed).toBeCloseTo(110, 0);
    expect(res.body.series[1].ipcaAccumulatedPct).not.toBeNull();
    expect(res.body.series[1].cdiAccumulatedPct).not.toBeNull();
  });
