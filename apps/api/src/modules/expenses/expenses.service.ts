...
import type { ExpenseSource } from "@money-manager/types";
...
export async function createBotExpense(input: CreateBotExpenseBody): Promise<Expense> {
  const amountCents = Math.round(input.amount * 100);
  const occurredAt = parseOccurredAt(input.occurredAt);
  const paymentMethod: PaymentMethod = "credit_card";
  const now = new Date();
  const source: ExpenseSource = input.source;
...
export async function countUncategorizedExpenses(): Promise<number> {
  const [row] = await db
    .select({ count: sql<number>`count(*)` })
    .from(expenses)
    .where(and(isNull(expenses.deletedAt), isNull(expenses.goalCategory)));
...
export async function listUncategorizedExpenses(
  ...
  const filters = [isNull(expenses.deletedAt), isNull(expenses.goalCategory)];
...
export async function categorizeExpense(
  ...
  if (existing.goalCategory !== null) {
    throw new ValidationError("Despesa já possui categoria");
  }
