...
**Postgres no Coolify — uma instância só.** O `docker-compose.coolify.yml` define um serviço `postgres` com `POSTGRES_USER` e `POSTGRES_DB` iguais a `money_manager`, publica `${POSTGRES_PUBLISH_PORT:-15432}:5432` para acesso externo (DBeaver, etc.) e usa `POSTGRES_PASSWORD` compartilhado com a `DATABASE_URL` da API. **Remova** no Coolify o recurso Database duplicado que apontava só para o banco `postgres` (outro container). Em produção defina `POSTGRES_PASSWORD` (e opcionalmente `POSTGRES_PUBLISH_PORT`) nas variáveis do stack. Fluxo antigo com dois Postgres: (1) agora: uma instância no compose + porta publicada; ou (2) alternativa: só Postgres gerenciado no Coolify e sem serviço `postgres` no compose — aí alinhar `DATABASE_URL` à URL interna do painel.

**Migrations no deploy:** a imagem da API (`apps/api/Dockerfile`) usa `docker-entrypoint.sh`, que executa `pnpm --filter @money-manager/db run db:migrate:runtime` (script `packages/db/scripts/run-migrations.mjs`) quando `DATABASE_URL` está definida e `RUN_DB_MIGRATIONS` não é `false`. O script registra `current_user`/`current_database` nos logs, reaplica DDL se detectar meta órfã (linhas em `drizzle.__drizzle_migrations` sem `public.users`) e valida tabelas com contagens em `information_schema`. Ao inspecionar o banco no DBeaver, use o **mesmo usuário** que em `DATABASE_URL`: no Postgres, `information_schema.tables` só reflete objetos acessíveis ao role da sessão — outro usuário pode parecer “schema public vazio” mesmo com tabelas criadas pela API. `RUN_DB_MIGRATIONS=false` evita corrida se você escalar a API para várias instâncias — nesse caso rode migrations uma vez por deploy apontando o mesmo `DATABASE_URL`.
